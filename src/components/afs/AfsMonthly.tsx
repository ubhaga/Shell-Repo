...
export function AfsMonthly({ selectedDate }: AfsMonthlyProps) {
  ...
  const [prevBankLines, setPrevBankLines] = useState<{ id: string; amount: number; description: string; transaction_date: string; matched_terminal: string }[]>([]);
  const [creditorOBs, setCreditorOBs] = useState<Record<string, number>>({});
  const [manualMatches, setManualMatches] = useState<{ month: string; cashup_date: string; terminal: string; bank_amount: number; bank_line_id: string }[]>([]);
  ...
  const loadData = useCallback(async () => {
    const [bankRes, prevBankRes, obRes, matchRes] = await Promise.all([
      supabase.from('bank_statement_lines').select('id, matched_terminal, amount, description, transaction_date').eq('month', month),
  ...
  const eftClearing = useMemo(() => {
    ...
    const speedpointByDate = monthlyCashups.map(c => {
      ...
      SP_TERMINALS.forEach(t => { termMap[t] = { batchNo: '', total: 0 }; });
    ...
    bankLines.forEach((l) => {
      ...
      bankParsed.push({ terminal: l.matched_terminal, batch, amount: l.amount, bankLineId: l.id });
    });
    ...
    const manuallyMatchedIds = new Set(currentManualMatches.map(m => m.bank_line_id));
    ...
    const speedpointMatches: Record<string, { bankAmount: number }>[] = speedpointByDate.map(r => {
      ...
      SP_TERMINALS.forEach(t => {
        ...
        if (!td || td.total === 0) { rowMatch[t] = { bankAmount: 0 }; return; }
    ...
    prevBankLines.forEach((l) => {
      ...
      const batchMatch = l.description.match(new RegExp(`${termNum}\\s+(\\d+)`));
      const batch = batchMatch ? batchMatch[1] : '';
      prevBankParsed.push({ terminal: l.matched_terminal, batch, amount: l.amount, bankLineId: l.id });
    });
    ...
    SP_TERMINALS.forEach(t => { obByTerminal[t] = { cashup: 0, bank: 0 }; });
  ...
  const creditors = useMemo(() => {
    ...
    const supplierByNormalized = new Map(
      allSuppliers.map((supplier) => [normalizeName(supplier), supplier])
    );
